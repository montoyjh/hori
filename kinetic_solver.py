...
class kinetic_solver:
    ...
    def initialize(self, NG, NS, fugacities, stoich_mat, k_r, K_r):
        ...
        stoich_mat is the stoichiometric matrix
        fugacities is a list of fugacities (must match the order in stoich_mat).
        """
    ...
    def get_jacobian_ci(self, coverages, t=0, verbose=0):
        ...
        of coverages.
        Has not been tested extesively.
    ...
    def get_jacobian_2_ci(self, coverages, t=0, verbose=0):
        """ Calculate the Jacobian d f_c / d theta_i
        The current implementation assumes the (effective) rate constants are independent
        of coverages. Has not been tested extesively.
        Delete.
        """
        if verbose > 0: print 'get_jacobian_ci; c,t:',  coverages, t
        J_ci = np.zeros((self.NS, self.NS))
                    ...
                    for c2 in range(self.NS): # loop over theta_c2
                        ...
                        if i==c2:
                            ...
                            if alpha < 0 :
                                r_forward *= -alpha
                                alpha = alpha + 1
                                #print i, c, r, c2, '\t', self.stoich_mat[r,c2+self.NG], alpha, theta**(-alpha)
                                r_forward *= theta**(-alpha)
                            elif alpha > 0 :
                                r_reverse *= alpha
                                alpha = alpha -1
                        ...
                        if alpha < 0.:
                            ...
                            r_reverse *= theta**(alpha)
                    # contribution corresponding to \partial fc / \partial thetai
                    J_ci[c,i] += self.stoich_mat[r,c+self.NG] * ( r_forward - r_reverse )
        return J_ci
...
class integrate_to_time_t_vode(kinetic_solver):
    """ Integrate the ODE's to through time t.
    using scipy's vode integrator (how do I reduce tolerance?)
    """
...
class steady_state_solver(kinetic_solver):
    ...
    KEEP.
    But implement a function accepting constraints.
    """
