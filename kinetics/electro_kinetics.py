...
class electro_kinetics:
    def __init__(self, rate_functions, path, at, pet, gt, voltage, T, solver):
        ...
        self.NG = len(self.gas_names) # number of non-adsorbed species (including the pe-pair)
    ...
    def set_fugacities(self, fugacity_dict):
        ...
        The fugacity is relative to the standard state defined for the gas thermodynamics object.
    ...
    def _calculate_k_r(self):
        ...
        for i in range(NR):
            ...
            k_r[i] = rf(U=self.voltage, T=self.T, U0 = U0)
        self.k_r = k_r
    ...
    def set_gas_standardstate(self, gas, fugacity, recalc=True):
        """ Change standard state for gas species (and the vapor pressure of liquid and aqueous species).
